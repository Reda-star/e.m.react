...
const ManagerEdit = () => {
    ...
    useEffect(() => {
        fetch("http://localhost:1016/api/manager/" + empid).then((res) => {
            ...
            idchange(resp.id);
            nomchange(resp.nom);
            prenomchange(resp.prenom);
            employeschange(resp.employes);
            departementchange(resp.departement);
    ...
    const handlesubmit=(e)=>{
      e.preventDefault();
      const empdata={id,nom,prenom,employes,departement,active};
      

      fetch("http://localhost:1016/api/manager/update/"+empid,{
        method:"PUT",
